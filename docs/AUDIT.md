...
**Status: all 14 items resolved in the 2026-08-07 tech-debt pass** (commits 896b787..HEAD), alongside the custom-activities feature that motivated the state-layer work. Per-item resolution notes below.
...
### [x] 1. Replace the live `S` proxy with explicit `getState()` calls
...
- **Resolved (2026-08-07):** S is now a deep read-only proxy over the store-synced view: every read works, any write at any depth throws. Rehydration writes the internal _S directly.

### [x] 2. Move module-level mutable globals into store state
...
- **Effort:** Medium.
- **Resolved (2026-08-07):** useSampleGear + userInventory live in the store; getInventory() derives the active set. All mutators dispatch (SET_SAMPLE_GEAR, ADD_ITEMS, SET_USER_INVENTORY, UPDATE_ITEM).

### [x] 3. Use event delegation instead of per-render handler caching
...
- **Effort:** Medium.
- **Resolved (2026-08-07):** Tap/long-press wiring is delegated to four stable containers, bound once at init with live isMobile() guards. The _mTapBound/_mHandler/_lpBound flags are gone.

### [x] 4. Add tests for pure functions
- **Where:** none exist.
...
- **Resolved (2026-08-07):** tests/ covers engine (store/rules/stats), the Quick Add parser + CSV, curated-data invariants, and xml.js build (+ browser-only DOMParser round-trip). 87 node / 89 browser tests.

---

## Medium impact

### [x] 5. Extract emoji picker data to its own file (+ dedupe)
...
- **Resolved (2026-08-07):** EP_DATA/EP_TABS moved to emoji-data.js; all 20 duplicate keys merged with united keyword sets. Build is warning-free.

### [x] 6. Move packing-list export template to a build-inlined file
...
- **Effort:** Small.
- **Resolved (2026-08-07):** Template moved to exports/packing-list.html, inlined via esbuild's text loader. Real script tags, placeholder substitution, and a stray mTapHint div removed.

### [x] 7. Stop direct-mutating store state in `importXML`
...
- **Effort:** Small.
- **Resolved (2026-08-07):** importXML dispatches ADD_ITEMS + INSTALL_LOADOUTS; parsing itself extracted to xml.js parseTrailkitDoc.

### [x] 8. Constants for action types
...
- **Resolved (2026-08-07):** const A = {...} action constants used at every dispatch and reducer registration.

### [x] 9. Surface localStorage quota failures
...
- **Effort:** Small.
- **Resolved (2026-08-07):** Persistence.save returns success; persistState shows one toast per session on failure.

---

## Low impact (cleanup, do opportunistically)

### [x] 10. Add a `VERSION` constant
...
- **Resolved (2026-08-07):** VERSION const in version.js feeds the About modal, mobile Etc tab, and XML/CSV/packing-list exports.

### [x] 11. Rename `bladderIds` → `bladderId`
...
- **Resolved (2026-08-07):** Renamed everywhere incl. SAMPLE/STARTER data; deserialize migrates stored payloads and loadout objects; LOAD_LOADOUT keeps a legacy-field belt.

### [x] 12. Centralize hardcoded element IDs
...
- **Resolved (2026-08-07):** Bounded resolution: all 246 lookups funnel through a $() chokepoint (one place for caching/diagnostics). A full id registry was judged more churn than value for a single-page app.

### [x] 13. Replace `alert()` with toasts
- `importXML` line ~3069 uses `alert()` for both success ("X items imported") and failure ("Import failed: ..."). Add toast helper to UIUtils.
- **Resolved (2026-08-07):** importXML's three alert()s became toasts in v1.15.

### [x] 14. Sort `SAMPLE_INVENTORY` and document the implied invariants
...
- **Resolved (2026-08-07):** Documented at the SAMPLE_INVENTORY head: ordering is cosmetic - renderStash sorts by TYPE_ORDER + name at render time (the audit's premise was stale).
